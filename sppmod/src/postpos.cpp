    ...
    int mode)
{
    ...
    while ((nobs = inputobs(obs, rtk.sol.stat, popt)) >= 0) {
        ...
        if (!rtkpos(&rtk, obs, n, &navs)) continue;///////////////////////////////处理数据
    ...
    char** infile, const int* index, int n, char* outfile)//
{
 ...
 /*   if (*fopt->dcb) {
    ...
    if (popt_.mode == PMODE_SINGLE || popt_.soltype == 0) {//单点定位
        if ((fp = openfile(outfile))) {
            procpos(fp, &popt_, sopt, 0); /* forward */////开始处理数据
            fclose(fp);
        }
    }
