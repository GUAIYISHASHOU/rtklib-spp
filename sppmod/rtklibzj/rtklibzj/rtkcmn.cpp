...
extern int satexclude(int sat, double var, int svh, const prcopt_t* opt)
{
    ...
    if (opt) {
        ...
        if (opt->exsats[sat - 1] == 2) return 0; /* included satellite *///通过配置文件删除
        if (!(sys & opt->navsys)) return 1; /* unselected sat sys */
    }
    if (sys == SYS_QZS) svh &= 0xFE; /* mask QZSS LEX health *///通过健康度删除卫星
    if (svh) {
        trace(3, "unhealthy satellite: sat=%3d svh=%02X\n", sat, svh);
        return 1;
    }
    if (var > MAX_VAR_EPH) {//方差大的删除
...
* return : matrix pointer (if n<=0 or m<=0, return NULL)
*-----------------------------------------------------------------------------*/
extern int* imat(int n, int m)//把所有变量设置为零
{
    int* p;
...
*          double *y        I   (weighted) measurements (m x 1)//增加了权重的
...
*-----------------------------------------------------------------------------*/
extern void ecef2pos(const double* r, double* pos)//xyz转BLH
{
...
/* geometric distance ----------------------------------------------------------
* compute geometric distance and receiver-to-satellite unit vector单位向量
* args   : double *rs       I   satellilte position (ecef at transmission) (m)
*          double *rr       I   receiver position (ecef at reception) (m)
*          double *e        O   line-of-sight vector (ecef)
* return : geometric distance (m) (0>:error/no satellite position)
* notes  : distance includes sagnac effect correction//地球自转改正
*-----------------------------------------------------------------------------*/
extern double geodist(const double* rs, const double* rr, double* e)//geometric distance几何距离
{
    double r;
    int i;

    if (norm(rs, 3) < RE_WGS84) return -1.0;//距离小于地球半径出错
    for (i = 0; i < 3; i++) e[i] = rs[i] - rr[i];
    r = norm(e, 3);//二范数（开根号，a*a‘，然后开根号）
    for (i = 0; i < 3; i++) e[i] /= r;//三个分量除以距离得到矩阵
    return r + OMGE * (rs[0] * rr[1] - rs[1] * rr[0]) / CLIGHT;//OMGE地球自转角速度
}
/* satellite azimuth/elevation angle -------------------------------------------
* compute satellite azimuth/elevation angle
* args   : double *pos      I   geodetic position {lat,lon,h} (rad,m)
*          double *e        I   receiver-to-satellilte unit vevtor (ecef)//方向余弦
...
extern double satazel(const double* pos, const double* e, double* azel)
{
    ...
    if (pos[2] > -RE_WGS84) {
        ecef2enu(pos, e, enu);//xyztoblh
        az = dot(enu, enu, 2) < 1E-12 ? 0.0 : atan2(enu[0], enu[1]);//arctant
        if (az < 0.0) az += 2 * PI;
        el = asin(enu[2]);
    }
...
{
    double H[4 * MAXSAT], Q[16], cosel, sinel;//通过高度角方位角算方向向量
    int i, n;
