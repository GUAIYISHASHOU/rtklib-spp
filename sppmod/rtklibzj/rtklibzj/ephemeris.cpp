...
*          satellite position and clock are values at signal transmission time信号发射时刻
    ...
    int ephopt, double* rs, double* dts, double* var, int* svh)
{
    ...
    for (i = 0; i < n && i < 2 * MAXOBS; i++) {
        ...
        time[i] = timeadd(obs[i].time, -pr / CLIGHT);//�ȼ�һ��
        ...
        time[i] = timeadd(time[i], -dt);//�ټ�һ��

        /* satellite position and clock at transmission time *///��������λ�ú�������
